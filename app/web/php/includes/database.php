...
class Database {
     ...
     *
     * Merges $config into whatever is already on disk instead of replacing
     * the file outright — the setup form only ever submits a subset of the
     * known settings (JELLYSTREAM_PUBLIC_URL, MEDIA_PATH_MAP,
     * PREFERRED_AUDIO_LANGUAGE, etc. aren't fields on the form), so a
     * wholesale overwrite would silently delete any of those every time the
     * wizard is submitted.
     */
    public function saveEnvConfig($config) {
        $env_file = __DIR__ . '/../../../../.env';

        $merged = $this->getEnvConfig();
